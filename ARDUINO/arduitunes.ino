...
void computeNotePeriods(){
  for (char i = 0; i < ACTIVE_VOICES_COUNT; i++){
    ...
    */
    ...
    current_note_half_periods_us[i] = half_period + ((half_period%mcd)<=(mcd/2) ? -(half_period%mcd) : (mcd-(half_period%mcd))); // mcd-adjusted period
  }
}
...
void loadNextVoiceNotestringToBuffer(char voice_index){
  ...
  if (current_notestring_indexes[voice_index] == -1){
    ...
    if (is_repetition_start||is_repetition_end){  // repetition symbol. Read next note
